...
public class Player {
    ...
    Hashtable<String, Long> valueTable=new Hashtable<String, Long>(64*64*64);
    long roundStartTime;
    ...
    public GameState play(final GameState gameState, final Deadline deadline) {
        ...
        roundStartTime=Deadline.getCpuTime();
       ...
       // System.err.println("currentStateValue:"+ evaluate(gameState, "test"));
         ...
         */
        long v0=Integer.MIN_VALUE;
        long bestValue=Integer.MIN_VALUE;
        int bestIdx=0;
        long stateValue;
        int searchDepth=2;
        long alpha0=Integer.MIN_VALUE;
        long beta0=Integer.MAX_VALUE;
        int playerIdx=1;
        ...
        for(int i=0;i<nextStates.size();i++){
            stateValue=alphaBeta(nextStates.elementAt(i),searchDepth,alpha0,beta0,playerIdx);
            ...
            System.err.println("getCpuTime "+Deadline.getCpuTime());
            System.err.println("getCpuTime-rt " + (Deadline.getCpuTime() - roundStartTime));
            if(Deadline.getCpuTime()-roundStartTime>600000)break;
            //if(Deadline.getCpuTime()-roundStartTime>8500000);
    ...
    long alphaBeta(GameState state,int depth,long alpha,long beta, int playerIdx){
        long v;
        //if(Deadline.getCpuTime()-roundStartTime>2000000){
          //depth = 0;
        //  }
        if(depth==0){
        ...
        System.err.println("depth "+depth+" score "+v);

        return v;
    }

      long evaluate(GameState s, int playerIdx){
...
//repurposed a bit of code from gameState
    long checkL(GameState s,int currentPlayerIdx, int row1, int col1, int layer1, int row2, int col2, int layer2) {
     int BOARD_SIZE=4;
     ...
     if (opponentPoints>0){
        if(playerPoints>0)return -((long)Math.pow(80,opponentPoints*2));
        return 0;
     }
    ...
    //  }
     ...
     return (long)Math.pow(80,playerPoints*2-1);
     }
...
//@Override
        public int compare(GameState sA, GameState sB){
            return (int)(evaluate(sB,1)- evaluate(sA,1));
        }
