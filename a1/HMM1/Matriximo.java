...
public class Matriximo {
  ...
  float getValue(float i,float j){ //wrong af
  ...
  Matriximo(String all){
    ...
    Matriximo(String all,String anystring){ //for vectors, can also look at the length of the data to determine if its a vector..
    String[] splittedString = all.split(" ");
    n = Integer.parseInt(splittedString[0]);
    m = 1;
    ...
    for (int i=0;i<n;i++){
      for (int j=0;j<m;j++){
        data[i][j] = Float.parseFloat(splittedString[i*m+j + 1]);
      }
    }
  }
